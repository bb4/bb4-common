...
object NiceNumbersSuite {
  ...
  private val EXPECTED_TIGHT_CUTS3 = Array("-1.5", "0", "2", "4", "4.61")
...
class NiceNumbersSuite extends FunSuite {
  ...
  test("NiceNumbers3") {
    ...
    generator.setUseTightLabeling(false)
    val resultLoose = generator.getCutPointLabels(range, 5)
    assert(EXPECTED_LOOSE_CUTS3 === resultLoose)

    generator.setUseTightLabeling(true)
    val resultTight = generator.getCutPointLabels(range, 5)
    assert(EXPECTED_TIGHT_CUTS3 === resultTight)
  }
  ...
  test("NiceNumbers4") {
    ...
    assert(EXPECTED_LOOSE_CUTS4 === resultLoose)
    assert(EXPECTED_TIGHT_CUTS4 === resultTight)
  }
